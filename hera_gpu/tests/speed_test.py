...
class TestSpeed(unittest.TestCase):
	...
	t = time.time()
	vis.vis_gpu(antpos, freq, eq2tops, crd_eq, I_sky, bm_cube)
	print "SMALL GPU TIME (FLOAT):", time.time() - t, "s"

	t = time.time()
	vis_cpu(antpos, freq, eq2tops, crd_eq, I_sky, bm_cube)
	...
	t = time.time()
	vis.vis_gpu(antpos, freq, eq2tops, crd_eq, I_sky, bm_cube, real_dtype=np.float64, complex_dtype=np.complex128)
	print "SMALL GPU TIME (DOUBLE):", time.time() - t, "s"

	t = time.time()
	vis_cpu(antpos, freq, eq2tops, crd_eq, I_sky, bm_cube, real_dtype=np.float64, complex_dtype=np.complex128)
	...
	t = time.time()
	vis.vis_gpu(antpos, freq, eq2tops, crd_eq, I_sky, bm_cube)
	print "350 ANTS, PI*10^6 PIX GPU TIME (FLOAT):", time.time() - t, "s"

	t = time.time()
	vis_cpu(antpos, freq, eq2tops, crd_eq, I_sky, bm_cube)
	print "350 ANTS, PI*10^6 PIX CPU TIME (FLOAT):", time.time() - t, "s"
	...
	t = time.time()
	vis.vis_gpu(antpos, freq, eq2tops, crd_eq, I_sky, bm_cube, real_dtype=np.float64, complex_dtype=np.complex128)
	print "350 ANTS, PI*10^6 PIX GPU TIME (DOUBLE):", time.time() - t, "s"

	t = time.time()
	vis_cpu(antpos, freq, eq2tops, crd_eq, I_sky, bm_cube, real_dtype=np.float64, complex_dtype=np.complex128)
	print "350 ANTS, PI*10^6 CPU TIME (DOUBLE):", time.time() - t, "s"'''
...
def vis_cpu(antpos, freq, eq2tops, crd_eq, I_sky, bm_cube, real_dtype=np.float32, complex_dtype=np.complex64):
    ...
    bm_pix_x = np.array(np.linspace(-1, 1, bm_pix), dtype=np.float32) #######################ALTERED
    bm_pix_y = np.array(np.linspace(-1, 1, bm_pix), dtype=np.float32) #######################ALTERED

    # Loop over time samples
    for t, eq2top in enumerate(eq2tops.astype(real_dtype)):
        tx, ty, tz = crd_top = np.array(np.dot(eq2top, crd_eq), dtype=np.float32) ###########ALTERED
        for i in range(nant):
            # Linear interpolation of primary beam pattern
            spline = RectBivariateSpline(bm_pix_y, bm_pix_x, bm_cube[i], kx=1, ky=1, bbox=[-1,1,-1,1])
